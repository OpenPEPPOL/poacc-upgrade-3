...
<pattern xmlns="http://purl.oclc.org/dsdl/schematron">
    ...
    <let name="TAXamount" value="if(/ubl:OrderChange/cac:TaxTotal/cbc:TaxAmount) then xs:decimal(/ubl:Order/cac:TaxTotal/cbc:TaxAmount) else 0"/>
        ...
        <rule context="cbc:ProfileID">
                <assert id="PEPPOL-T114-R031"
                        ...
                        flag="fatal">An order change transaction SHALL use profile advanced_ordering.</assert>
        </rule>

		<rule context="cbc:CustomizationID">
				<assert id="PEPPOL-T114-R034"
						test="starts-with(normalize-space(.), 'urn:fdc:peppol.eu:poacc:trns:order_change:3')"
						flag="fatal">Specification identifier SHALL start with the value 'urn:fdc:peppol.eu:poacc:trns:order_change:3'.</assert>
		</rule>
    ...
    <rule context="cac:OriginatorCustomerParty">
        <assert id="PEPPOL-T114-R014"
                ...
                flag="fatal">An order change SHALL have the originator party name or an identifier</assert>
    </rule>
