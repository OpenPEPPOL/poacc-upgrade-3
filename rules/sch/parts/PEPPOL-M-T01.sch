...
<pattern xmlns="http://purl.oclc.org/dsdl/schematron">
    ...
    <let name="VATamount" value="if (/ubl:Order/cac:TaxTotal/cbc:TaxAmount) then xs:decimal(/ubl:Order/cac:TaxTotal/cbc:TaxAmount) else 0"></let>
    ...
    <rule context="cac:AnticipatedMonetaryTotal">
        ...
        <assert id="PEPPOL-T01-R006"
            ...
            flag="fatal">Expected total amount for payment SHALL NOT be negative
        </assert>
        <assert id="PEPPOL-T01-R007"
            test="$lineEtensionAmount &gt;=0"
            flag="fatal">Expected total sum of line amounts SHALL NOT be negative
        </assert>
        <assert id="PEPPOL-T01-R008"
            test="$lineEtensionAmount = $sumLineExtensionAmount"
            flag="fatal">Expected total sum of line amounts SHALL equal the sum of the order line amounts at order line level
        </assert>
        <assert id="PEPPOL-T01-R009"
            test="$allowanceTotalAmount = $sumAllowance"
            flag="fatal">Expected total sum of allowance at document level SHALL be equal to the sum of allowance amounts at document level
        </assert>
        <assert id="PEPPOL-T01-R010"
            test="$chargeTotalAmount= $sumCharge"
            flag="fatal">Expected total sum of charges at document level SHALL be equal to the sum of charge amounts at document level
        </assert>
    ...
    <rule context="cac:OrderLine/cac:LineItem">
        ...
        <assert id="PEPPOL-T01-R001"
            ...
            flag="fatal">Each order line SHALL have a document line identifier that is unique within the order.
        </assert>
        <assert id="PEPPOL-T01-R004"
            test="number(cbc:Quantity) &gt;=0"
            flag="fatal">Each order line ordered quantity SHALL not be negative
        </assert>
    ...
    <rule context="cac:Item">
        <assert id= "PEPPOL-T01-R012"
            ...
            flag="fatal">Each order line SHALL have an item identifier and/or an item name
        </assert>
    </rule>
