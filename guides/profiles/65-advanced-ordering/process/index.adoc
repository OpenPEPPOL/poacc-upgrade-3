...
* The seller may also change the order by sending an order response with proposed changes.
* The buyer may cancel the order by sending an order cancellation which may be accepted or rejected by the seller.
* The seller may cancel the order by sending a negative order response with rejection of all lines.
