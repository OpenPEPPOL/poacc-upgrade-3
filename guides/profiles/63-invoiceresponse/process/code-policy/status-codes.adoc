...
PD + PPD is the case when the status is PD accompanied with a Clarification reason code PPD. In this case the status is that an invoice is partially paid with the intention of paying the full invoice amount that was accepted.
